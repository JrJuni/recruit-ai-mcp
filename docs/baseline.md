...
| `analyze_deal` | `deal_id` | None | `ok`, `deal_id`, `analysis`, `product_context_used`, `product_context_ref_count`, `product_context_refs`, `warnings`, `usage`, `usage_summary` | Calls LLM and attempts to persist `bd_strategy`, `bd_strategy_usage`, and seller-side `bd_strategy_product_context_refs`; analysis still returns if that save fails. If product context is indexed and enabled, it may retrieve bounded seller-side snippets before strategy generation. It stores only refs metadata, never raw product text |
