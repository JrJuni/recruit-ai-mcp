...
|---|---|---|---|---|
| `config_doctor` | None | `offline` | `ok`, `profile`, `generated_at`, `summary`, `checks`, `next_actions`, `first_data_next_steps` | Read only; checks config, storage readiness, vector-search mode, and LLM provider readiness without LLM calls, embeddings, or writes. The default path may perform a bounded storage ping; `offline=true` skips it. When MongoDB URI is missing in full/pro mode, the `mongodb_uri` check returns a secret-safe hint that explains Atlas Free/M0 setup and asks whether to continue in zero-config sample mode for now. When checks pass, `first_data_next_steps` stays surface-aware: `local_sample` points a new user to create local personal recruiting records in `recruiting.json`, while Mongo-backed full/pro points a new user to create real client, position, candidate, recruiting evidence, and the first deterministic recommendation |
...
| `migrate_local_data` | None | `target_database`, `confirmed_by_user`, `dry_run`, `overwrite` | `ok`, `migration_type`, `dry_run`, `storage_written`, `source`, `target`, `options`, `counts`, `deals`, `recruiting`, `warnings` | Migrates user-created local personal deal and recruiting records from `storage.local_data_dir` to MongoDB. Defaults to dry-run, requires confirmation for writes, skips existing target record ids unless `overwrite=true`, and never migrates bundled fixture records or local delete audit logs |
