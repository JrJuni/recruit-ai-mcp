...
|---|---|---|---|---|
| `config_doctor` | None | `offline` | `ok`, `profile`, `generated_at`, `summary`, `checks`, `next_actions`, `first_data_next_steps` | Read only; checks config, storage readiness, vector-search mode, and LLM provider readiness without LLM calls, embeddings, or writes. The default path may perform a bounded storage ping; `offline=true` skips it. When MongoDB URI is missing in full/pro mode, the `mongodb_uri` check returns a secret-safe hint that explains Atlas Free/M0 setup and asks whether to continue in zero-config sample mode for now. When checks pass, `first_data_next_steps` stays surface-aware: `local_sample` points to sample-safe read/metric tools, while Mongo-backed full/pro points a new user to create a client company, position, candidate, add recruiting evidence, and run the first deterministic recommendation |
