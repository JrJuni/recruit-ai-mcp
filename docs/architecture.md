...
[deal-intel-mcp 서버  18 tools]
         ...
         │    ├── OpenAI API      gpt-5.5 등 Responses API 모델 (선택)
...
| `update_deal` | 없음 | 없음 | 사용자 확인 후 deal value·metadata 필드를 수정하고 history 기록 |
...
src/deal_intel/
  ...
  providers/
    llm.py              LLMProvider ABC
                        ...
                        OpenAIAPIProvider   — httpx, official Responses API
  ...
  tools/
    ...
    update_deal.py      확인된 deal_size_*·metadata 필드 수정 → history append
...
provider = cfg["llm"]["provider"]   # "chatgpt_oauth" | "openai_api" | "anthropic"
explicit_env = os.environ.get("DEAL_INTEL_LLM_PROVIDER")
...
- model: `llm.openai_api_model`
- reasoning effort: `llm.openai_api_reasoning_effort`; `none` 또는 빈 값은
  reasoning 파라미터를 보내지 않는다.
- 호출 표면: OpenAI Responses API `POST /v1/responses`, `store: false`
