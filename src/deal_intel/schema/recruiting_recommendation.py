...
def _risk_flags(
    ...
    if _has_process_conflict(fit) and "process_conflict" not in flags:
...
def _has_process_conflict(fit: CandidatePositionFitResult) -> bool:
    risk_signal = fit.signals["risk"]
    return "Confirm competing process or offer-deadline plan." in risk_signal.missing_info
