...
도구 목록에 아래 11개가 보이면 완료.

```
create_deal / add_meeting / get_deal / update_stage
list_deals / get_metrics / get_insights / get_customer_themes
...
### 7. `get_metrics` — 현재 파이프라인 건강도 KPI
...
### 8. `export_report` — 주간 파이프라인 보고서 생성
...
### 11. `get_customer_themes` — 고객 고민·선정 기준 빈도
...
[Claude Desktop / Codex — 자연어 입력]
         │ stdio JSON-RPC
         ▼
[deal-intel-mcp  FastMCP 서버  11 tools]
         │
         ├── LLM Provider
