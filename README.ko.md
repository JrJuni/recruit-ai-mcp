...
`deal-intel-mcp`는 1인 창업자, 초기 AI 팀, 가벼운 BD 팀을 위한 self-owned
deal memory layer다.

미팅 노트, 고객 이메일 답장, 사용자 인터뷰, 콜 요약, 내부 메모를 구조화된 딜
신호로 바꾼다. 무엇을 알고 있는지, 무엇이 비어 있는지, 어떤 고객사를 먼저 봐야
하는지, 다음 대화 전에 무엇을 확인해야 하는지 Claude Desktop, Codex, ChatGPT 같은
MCP host에서 바로 물어볼 수 있게 한다.

성숙한 영업 조직에게는 Salesforce 대체재가 아니다. 하지만 현재 시스템이 흩어진
메모, 스프레드시트, 이메일 thread, 사람 기억이라면 초기 팀의 첫 CRM-like layer로
쓸 수 있다. 데이터는 사용자의 MongoDB Atlas 또는 로컬 저장소에 남고, LLM provider도
사용자가 고른다.

기본 운영 경로는 MongoDB Atlas 기반 `full` 모드다. 무료/M0 tier로도 시작할 수
있다. MongoDB 없는 `sample` 모드는 AI agent의 빠른 평가, zero-config trial, 데모용
보조 경로다. 실제 팀 데이터 운영은 `full`에서 시작하는 것을 권장한다.

처음 볼 문서:

- AI와 함께 설치하기: [`AI_START_HERE.md`](AI_START_HERE.md)
- 자세한 full-mode 설치 가이드: [`AI_FULL_INSTALL_GUIDE.md`](AI_FULL_INSTALL_GUIDE.md)
- 공개 데모 질문: [`docs/public-demo-script.md`](docs/public-demo-script.md)
- 첫 외부 테스터용 handoff: [`AI_USER_TEST_GUIDE.md`](AI_USER_TEST_GUIDE.md)
- 포크/커스터마이즈: [`docs/extending.md`](docs/extending.md),
  [`docs/customization-recipes.md`](docs/customization-recipes.md),
  [`docs/architecture.md`](docs/architecture.md)
- 한국어 full-mode 설치 가이드: [`AI_FULL_INSTALL_GUIDE.ko.md`](AI_FULL_INSTALL_GUIDE.ko.md)

---

## 무엇을 하나요?

- 딜 기록과 고객 evidence를 사용자의 MongoDB Atlas 또는 로컬 sample/personal
  storage에 저장한다.
- messy한 고객 발언을 구조화된 딜 필드, health signal, follow-up gap, customer
  theme, 주간 리뷰 산출물로 바꾼다.
- ICP, 포지셔닝, 가격 정책, 보안 주장, 연동, 경쟁 비교 같은 seller-side
  product/solution context를 추가해 interaction 추출이 제품을 더 잘 이해하게 한다.
- AI host가 "어떤 딜을 먼저 봐야 해?", "고객들이 가장 많이 걱정한 건 뭐야?",
  "이번 주 파이프라인 보고서 만들어줘" 같은 자연어 질문에 답하게 한다.
- read-only BI, 리뷰, 보고서, export 경로는 가능한 LLM-free로 유지해, 서버는
  계산하고 host app은 설명하게 한다.
- `config_doctor`, storage check, usage/cost summary처럼 setup과 data-quality
  문제를 숨기지 않고 드러낸다.

## 무엇이 아닌가요?

- 자동으로 딜을 닫아주는 영업 자동화 도구가 아니다. evidence를 구조화하고 판단은
  사람이 한다.
- 권한, contact ownership, calendar/email sync, workflow automation, enterprise
  integration을 모두 갖춘 성숙한 CRM suite는 아직 아니다.
- 딜 데이터를 가져가는 hosted SaaS가 아니다. 기본 `full` 경로는 사용자의 MongoDB
  Atlas project와 사용자가 고른 LLM provider를 쓴다.
- 검증되지 않은 "매출 상승" 수치를 주장하지 않는다. 데이터가 부족하면 부족하다고
  말한다.

## 한눈에 보는 아키텍처

단순 prompt wrapper가 아니라 MCP로 노출되는 작은 deal-intelligence backend다.
...
일반적인 write path는 이렇다.

```text
1. meeting note, email reply, interview, call summary가 add_interaction으로 들어온다.
2. 서버 측 LLM이 고객 evidence에서 구조화된 딜 신호를 추출한다.
3. raw evidence, source metadata, derived qualification field가 저장된다.
4. deal summary와 product-context chunk는 검색을 위해 embedding될 수 있다.
5. read-only 도구가 metric, gap, report, theme, export를 추가 LLM 없이 계산한다.
6. MCP host가 최종 답변을 사용자에게 설명한다.
```

중요한 경계:

- MCP는 interface layer이고, Claude/Codex/ChatGPT가 UI가 될 수 있다.
- 고객 evidence와 health, gap, theme, qualification snapshot 같은 derived
  intelligence는 분리된다.
- LLM-heavy path는 주로 write/enrichment 쪽이다. read/report/export path는 가능한
  deterministic하게 유지한다.
- product context는 seller-side RAG context다. 해석을 돕지만 customer-stated
  evidence로 계산하지 않는다.
- `sample -> full -> pro`가 확장 경로다. zero-config trial, MongoDB-backed real
  data, 의도적인 paid infra의 Atlas Vector Search 순서다.
- tool surface는 profile별로 필터링되어 일반 사용자, sample 사용자, maintainer가
  같은 도구 목록을 볼 필요가 없다.

더 자세한 모듈 map은 [`docs/architecture.md`](docs/architecture.md)를 본다. 포크와
커스터마이즈 진입점은 [`docs/extending.md`](docs/extending.md)와
[`docs/customization-recipes.md`](docs/customization-recipes.md)에 있다.
...
Active/Attention 딜 수, 단계별 파이프라인 금액, qualification health band 분포, gap 분포, Open 파이프라인 금액을 한 화면에. 각 차트의 aggregation 파이프라인은 `render-atlas-dashboard` CLI로 생성해 Atlas Charts에 붙여넣는다 (아래 "Atlas Charts Dashboard" 섹션 참고).

### 2. Claude / Codex 인-챗 분석 렌더링

![Claude 인-챗 렌더링 대시보드](docs/images/chat-dashboard.png)

MCP 도구 결과를 그대로 받아 win rate, stage funnel, qualification gap pattern, data quality coverage, attention items까지 대화 안에서 렌더링한다. 별도 앱 없이 회의록 한 건 붙여넣는 것에서 시작한다.
...
내부적으로는 active qualification framework를 사용해 messy한 고객 evidence를 비교 가능한 딜 신호로 바꾼다. 기본 내장 framework는 B2B 영업에서 쓰이는 **MEDDPICC**이고, QF-v2부터는 preset을 복사해 팀의 영업 방식에 맞는 custom framework를 만들 수 있다.

사용자가 반드시 영업 전문가일 필요는 없다. 출력은 health, 빠진 정보, risk signal, customer theme, 다음에 물어볼 질문 중심으로 보여준다.
...
고객 evidence를 추가하면 서버 측 LLM이 framework 신호를 추출하고 구조화된 결과를 저장한다. `full` 모드에서는 실제 딜 데이터를 MongoDB Atlas에 저장해 패턴 분석까지 한다. 선택적인 `sample` 모드에서는 AI agent나 평가자가 설정 없이 기능을 볼 수 있도록 내장 가상 데이터로 같은 읽기/리뷰 화면을 제공한다.

---

## 제품/솔루션 컨텍스트

고객 evidence만으로는 "우리 제품이 무엇을 해결하는지"를 LLM이 정확히 알기 어렵다. 그래서 별도의 seller-side product context layer가 있다.

예를 들어 다음 자료를 넣을 수 있다.

- 제품 소개, ICP, 가격 정책
- 보안/컴플라이언스 주장
- 주요 integration, migration 방식
- 경쟁사 비교, 부적합 조건
- 제안서나 카탈로그에서 추출한 제품 설명

이 context는 고객이 말한 evidence로 계산되지 않는다. qualification score나 customer theme을 직접 올리지 않고, `add_interaction`이나 `analyze_deal`이 제품 fit, 용어, 가치 제안, disqualifier를 더 잘 해석하도록 돕는 보조 RAG layer다.

---

## Fork And Customize

이 레포는 MIT license로 제공된다. 출처와 license notice를 남기면 자유롭게 fork, 수정, 재배포, 내부 도구화할 수 있다.

특히 이런 팀에게 맞다.

- 일반 CRM은 너무 무겁지만 notes/sheets에 딜 운영을 맡기기엔 위험한 B2B SaaS/AI 스타트업
- MEDDPICC 대신 BANT, SPICED, 자체 qualification framework를 쓰고 싶은 개발자
- Claude Desktop, Codex, ChatGPT 같은 MCP workflow로 딜 데이터를 읽고 쓰고 싶은 팀
- 컨설팅, SI, 에이전시처럼 미팅·제안·견적·리스크 메모가 흩어지는 조직

확장할 때는 다음 문서부터 보면 된다.

- [`docs/extending.md`](docs/extending.md): 어디부터 고칠지, 어떤 contract를 지킬지
- [`docs/customization-recipes.md`](docs/customization-recipes.md): BANT 교체, custom field, 새 MCP tool, report 변경 recipe
- [`docs/architecture.md`](docs/architecture.md): 내부 모듈과 data pipeline map
...
**빠른 경로 — npx bootstrapper**

git clone 없이 먼저 써보고 싶다면 Node.js 18+와 Python 3.11+를 준비한 뒤
npx bootstrapper를 사용할 수 있다.

```bash
npx deal-intel-mcp@0.2.2 setup --python <python-3.11+-path>
npx deal-intel-mcp@0.2.2 smoke --profile-only
npx deal-intel-mcp@0.2.2 mcp-config --json
```

이 경로는 `~/.deal-intel/runtime/venv` 아래에 관리형 Python runtime을 만들고
PyPI의 `deal-intel-mcp[embedding]` 패키지를 설치한다. Claude Desktop MCPB에는
`mcp-config` 또는 `where`가 출력하는 Python interpreter path를 넣으면 된다.
코드를 수정하거나 커스텀 도구를 개발할 사람은 아래 editable install 경로를 쓰면
된다.
...
MCP 도구 목록이 로드되면 완료다. 현재 서버는 프로필에 따라 다른 tool surface를
노출한다. 정확한 최신 도구 수는 `get_tool_catalog`, `config_doctor`, 또는
`deal-intel config show`로 확인한다. 현재 계약 기준은 `sample=24`,
`standard=38`, `developer=41`이다.
...
- `qualification_latest` — 활성 qualification framework 기준의 누적 health_pct,
  coverage, uncertainty, gaps
- `meddpicc_latest` — 기본 MEDDPICC framework와 legacy record를 위한
  compatibility alias
...
- 도구 surface: `src/deal_intel/tool_surfaces.py`
- 현재 도구 수는 `get_tool_catalog`, `config_doctor`, 또는
  `deal-intel config show`로 확인한다. 현재 계약 기준은 `sample=24`,
  `standard=38`, `developer=41`이다.
...
[Claude Desktop / Codex — 자연어 입력]
         │ stdio JSON-RPC
         ▼
[deal-intel-mcp  FastMCP 서버  profile-filtered tools]
         │
         ├── LLM Provider
...
```
src/deal_intel/
  mcp_server.py         FastMCP 진입점, profile별 tool surface 필터링
